...
public class StorageService {
    ...
    public String uploadSingleImg(MultipartFile file, String Path){
        try {
            Path = Path.isEmpty()? publicPath+"imgproduct" : Path;
            MultipartFile[] arrayfile = {file};
    ...
    public List<String> uploadMultipleImages(MultipartFile [] files, String Path){
        Path = Path.isEmpty()? publicPath+"imgproduct" : Path;
        ...
        for(MultipartFile file : files) {
            try {
                ...
                String[] fname = file.getOriginalFilename().split(Pattern.quote("."));
                String filename = Helpers.randomString(25)+"."+fname[fname.length-1];
                String filename_thumb = "thumb_"+filename;
                String filename_thumb_small = "thumb_small_"+filename;
                ...
                BufferedImage croppedImage = resizeImage(file.getBytes(), max_width, max_height, crop);
                ...
                BufferedImage croppedImage2 = resizeImage(file.getBytes(), max_width, max_height, crop);
